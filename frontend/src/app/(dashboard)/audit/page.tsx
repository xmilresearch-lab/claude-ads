...
function ExpandedMetaRow({ entry }: { entry: AuditLogEntry }) {
  return (
    <tr className="bg-[#0A0B0F]">
      <td colSpan={6} className="border-b border-border px-6 py-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {isLoading && (
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-2xs font-mono text-text-muted">
                      Loading…
                    </td>
                  </tr>
                )}
                ...
                {data?.items.map((entry) => (
                  <Fragment key={entry.id}>
                    ...
                    {expandedId === entry.id && <ExpandedMetaRow entry={entry} />}
                  </Fragment>
                ))}
              </tbody>
            </table>
