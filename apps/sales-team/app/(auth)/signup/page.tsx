...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    e.preventDefault()
    setLoading(true)
    // Magic link via Resend — NextAuth creates the user on first sign-in
    await signIn('resend', { email, name, redirect: false })
    setSent(true)
    setLoading(false)
  }
      ...
      <main className="min-h-screen flex items-center justify-center bg-black">
        <div className="max-w-md w-full px-6 text-center">
          ...
          <p className="text-gray-400">
            We sent a magic link to <span className="text-white">{email}</span>. Click it to activate your account and get your first free analysis.
          </p>
        </div>
      </main>
    )
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-black">
      <div className="max-w-md w-full px-6">
        <h1 className="text-3xl font-bold text-white mb-2">Analyze your first offer free</h1>
        <p className="text-gray-400 mb-8">3 free analyses. No credit card required.</p>
        ...
        <form onSubmit={handleSignup} className="space-y-4 mb-6">
          ...
          >
            {loading ? 'Creating account...' : 'Get My Free Analysis'}
          </button>
        </form>
